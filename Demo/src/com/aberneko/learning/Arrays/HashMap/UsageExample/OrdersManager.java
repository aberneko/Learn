...
public class OrdersManager {
    ...
    void printCustomerOrders(String customerName) {
        if (customersOrders.containsKey(customerName)) { // проверьте, есть ли указанный ключ в таблице
            // customerName — это строка (String), которую вы используете для поиска.
            // customersOrders.get(customerName) — это значение (ArrayList<Double>), связанное с этой строкой.
            System.out.println("Заказы " + customerName + ":");
            System.out.println(customersOrders.get(customerName)); // когда вы передаёте любой объект (в данном случае,
            // ваш ArrayList) в метод System.out.println(), Java автоматически вызывает специальный метод: toString().
            // Метод toString() предназначен для возврата строкового представления объекта.
        }
    }
    ...
    String getMaxOrderCustomerName() {
        ...
        for (String name : customersOrders.keySet()) { // обходим список по ключам - именам, name - ключ,
            ArrayList<Double> value = customersOrders.get(name); // сохраняет ссылку на список заказов для клиента,
                                                                 // чьё имя находится в переменной name.
            // customersOrders.get(name) - возвращает значение, связанное с этим ключом. А значение это сам объект
            double currentSum = 0;
            for (double orderPrice : value) {
                currentSum += orderPrice; // складываем траты;
            }

            if (currentSum > maxOrder) {
                maxOrder = currentSum; // Обновляем максимальную сумму
                customerName = name; // Запоминаем имя клиента
            }
    ...
    void removeUnprofitableOrders() {
        ArrayList<String> names = new ArrayList<>(); // создайте список клиентов с заказами меньше 5000 для удаления

        // наполните список names
        for (String name : customersOrders.keySet()) {
            ArrayList<Double> orders = customersOrders.get(name);
            double ordersSum = 0;
            for (double sum : orders) {
                ordersSum += sum;
            }
